...
# Define the CSS style for the background color
#background_color = """
#    <style>
#    .stApp {
#        background-color: #009999; /* Teal color */
#    }
#    </style>
#    """
...
def calculate_taxes(filing_status, state, income, deductions, age, dependents, other_income, education_expenses, home_ownership, mortgage_paid, property_taxes_paid, donations, retirement_contributions):
    brackets = {
        ...
        "Married Filing Separately": [(0, 11000, 0.1), (11001, 44725, 0.12), (44726, 95375, 0.22), (95376, 182100, 0.24), (182101, 231250, 0.32), (231251, 346875, 0.35), (346876, float('inf'), 0.37)],
    ...
    if age >= 65:
        additional_deduction = 1850 if filing_status in ["Single", "Head of Household"] else 1350

    total_income = income + other_income
    total_deductions = deductions + education_expenses + donations + retirement_contributions + standard_deductions[filing_status] + additional_deduction
    
    if home_ownership:
        total_deductions += mortgage_paid + property_taxes_paid
...
# App title
st.title("Vialto Partners Tax Calculator 2023")
...
with col1:
    st.image("C:/Users/EA0438/Documents/Projects/Google/Tax Planning Tool/New folder/Picture1.png", use_column_width=True)  # Reemplaza "path/to/your/image.png" con la ruta de tu imagen

# Add form and results to the second column
with col2:
    ...
    if not filing_status or age is None or dependents is None or not state:
        ...
        if home_ownership:
            mortgage_paid = st.number_input("Annual Mortgage Paid (USD)", min_value=0, help="Enter your total annual mortgage paid.")
            property_taxes_paid = st.number_input("Annual Property Taxes Paid (USD)", min_value=0, help="Enter your total annual property taxes paid.")
        ...
        if st.button("Calculate"):
            tax, details, taxable_income, total_deductions = calculate_taxes(filing_status, state, income, deductions, age, dependents, other_income, education_expenses, home_ownership, mortgage_paid, property_taxes_paid, donations, retirement_contributions)
